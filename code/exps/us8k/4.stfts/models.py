...
#ESC50_MODEL_hybrid
class us8k_stft_model_hybrid(nn.Module):
  def __init__(self, mode, input_shape, batch_size=16, num_cats=50):
    ...
    if (mode=='exp4'):
      self.dense1 = nn.Linear(135680,500)  #exp4 MODE
    elif (mode=='exp8'):
...
#exp 8 model 2
class us8k_stft_model_hybrid2(nn.Module):
  def __init__(self, mode, input_shape,batch_size=16, num_cats=50):
    super().__init__()
    self.conv1 = nn.Conv2d(1, 32, kernel_size = 3, stride=1, padding=1)
    self.bn1 = nn.BatchNorm2d(32)
...
class us8k_stft_model_hybrid3(nn.Module):
  def __init__(self,mode, input_shape, batch_size=16, num_cats=50):
    super().__init__()
...
class hybridstftsap2dcnn(nn.Module):
  def __init__(self, batch_size=1, num_cats=10):
    ...
    self.bn6 = nn.BatchNorm2d(128)
    self.conv7 = nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=2)
    self.bn7 = nn.BatchNorm2d(256)
...
def load_model(model_name, mode):
  if (mode=='exp4'):
    model = us8k_stft_model_hybrid(input_shape=(1,513,431), batch_size=16, num_cats=50)
  elif(mode=='exp8'):
    model = us8k_stft_model_hybrid(input_shape=(1,257,431), batch_size=16, num_cats=50)
