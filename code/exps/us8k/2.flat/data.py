...
def get_features(mode, wavname):    
  ...
  '''
  if image.shape[0]==80 and image.shape[1]<(image.shape[0]//4):
    image = np.pad(image, [(0, 0), (0, image.shape[1])], mode='constant', constant_values=0)
  '''
  return image

class US8KData(Dataset):
  def __init__(self, df, in_col, out_col, mode):
    self.df = df
    self.data = []
    self.labels = []
    self.c2i={}
    ...
    for ind in range(len(df)):
      ...
      #FLATTEN FEATURES
      flatmel=np.ndarray.flatten((get_features(mode, row['slice_file_name'])))[np.newaxis,...] 
