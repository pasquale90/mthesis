...
'''
if(mode=='80'):
  model = model.esc_mel_model_hybrid(mode, input_shape=(1,80,431), batch_size=16, num_cats=50).to(device)
elif(mode=='128'):
  model = model.esc_mel_model_hybrid(mode, input_shape=(1,128,431), batch_size=16, num_cats=50).to(device)
'''
'''
model = model.esc_mel_model_hybrid(input_shape(1,int(mode),431), batch_size=16, num_cats=50, mode=mode).to(device)
'''
model =  models.hybridmelap2dcnn(batch_size=16,num_cats=50).to(device)
