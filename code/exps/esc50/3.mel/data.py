...
class ESC50Data(Dataset):
  def __init__(self, df, in_col, out_col, mode):#(train, 'filename', 'category', mode)
    ...
    for ind in range(len(df)):
      ...
      self.data.append(get_features(mode, row['filename'])[np.newaxis,...])#shape(1,80,431) | (1,128,431)
...
def get_features(wavname):
  name= paths.featurepath128 + wavname.split('.')[0]+'.png'
  image=Image.open(name)
  image=np.array(image)
  image = image/255.0
  return image
...
class ESC50Data(Dataset):
  def __init__(self, df, in_col, out_col):#(train, 'filename', 'category')
    ...
    for ind in range(len(df)):
      ...
      #file_path = os.path.join(audiopath,row[in_col])
      #self.data.append(spec_to_image(get_melspectrogram_db(file_path))[np.newaxis,...])
      self.data.append(get_features(row['filename'])[np.newaxis,...])#shape(1,80,431) | (1,128,431)
      self.labels.append(self.c2i[row['category']])
  def __len__(self):
    return len(self.data)
  def __getitem__(self, idx):#load data on demand
